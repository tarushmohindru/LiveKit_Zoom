...
const EnhancedDashboard = () => {
  ...
  const { isRecording, error: recordingError, startRecording, stopRecording } = useRecording({
    roomName: activeRoom?.name || '',
    onRecordingComplete: (url) => {
      toast.success('Recording completed! The video will be available shortly.');
      // Add the recording to localStorage
  ...
  const startMeeting = () => {
    ...
    // If recording was enabled in the modal, start recording after joining the room
    if (shouldStartRecording) {
      // We'll start the recording after a short delay to ensure the room is ready
      setTimeout(async () => {
        try {
  ...
  return (
    ...
    >
                  ...
                  <div className="modal-options">
                    <label className="modal-checkbox-label">
                      <input
                        ...
                        onChange={() => setShouldStartRecording(!shouldStartRecording)}
                        className="modal-checkbox"
                      />
                      <span className="checkbox-text">
                        <FaRecordVinyl className="checkbox-icon" /> Record this meeting
                      </span>
                    </label>
                  </div>
                  <motion.button 
      ...
      {activeRoom && (
        <div className="active-meeting-controls">
          <button
          ...
          >
            {isRecording ? (
              <>
                <FaStop className="button-icon" /> Stop Recording
              </>
            ) : (
              <>
                <FaRecordVinyl className="button-icon" /> Start Recording
              </>
            )}
          </button>
          <button onClick={handleEndMeeting} className="end-meeting-button">
            <FaTimes className="button-icon" /> End Meeting
          </button>
        </div>
      )}
    </motion.div>
